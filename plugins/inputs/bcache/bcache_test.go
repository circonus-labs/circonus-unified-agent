...
var (
	testBcachePath           = os.TempDir() + "/circonus/sys/fs/bcache"
	testBcacheUuidPath       = testBcachePath + "/663955a3-765a-4737-a9fd-8250a7a78411"
	testBcacheDevPath        = os.TempDir() + "/circonus/sys/devices/virtual/block/bcache0"
	testBcacheBackingDevPath = os.TempDir() + "/circonus/sys/devices/virtual/block/md10"
)
...
func TestBcacheGeneratesMetrics(t *testing.T) {
	...
	err = os.RemoveAll(os.TempDir() + "/circonus")
	require.NoError(t, err)
}
