...
type MQTTConsumer struct {
	...
	sem           semaphore
	messages      map[cua.TrackingID]bool
	topicTag      string
...
func (m *MQTTConsumer) connect() error {
	...
	m.state = Connected
	m.messages = make(map[cua.TrackingID]bool)
...
func (m *MQTTConsumer) createOpts() (*mqtt.ClientOptions, error) {
	...
	if m.ClientID == "" {
		opts.SetClientID("Circonus-Consumer-" + internal.RandomString(5))
	} else {
	...
	for _, server := range m.Servers {
		// Preserve support for host:port style servers; deprecated in v1.4.4
...
func init() {
	inputs.Add("mqtt_consumer", func() cua.Input {
