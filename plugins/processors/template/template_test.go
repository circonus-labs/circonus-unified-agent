...
func TestTagTemplateConcatenate(t *testing.T) {
	...
	input := []cua.Metric{testutil.MustMetric("Tags", map[string]string{"hostname": "localhost", "level": "debug"}, nil, now)}

	// act
	actual := tmp.Apply(input[0])

	// assert
	expected := []cua.Metric{testutil.MustMetric("Tags", map[string]string{"hostname": "localhost", "level": "debug", "topic": "localhost.debug"}, nil, now)}
	testutil.RequireMetricsEqual(t, expected, actual)
}
...
func TestTagAndFieldConcatenate(t *testing.T) {
	...
	// assert
	expected := []cua.Metric{testutil.MustMetric("weather", map[string]string{"location": "us-midwest", "LocalTemp": "us-midwest is too warm"}, map[string]interface{}{"temperature": "too warm"}, now)}
	testutil.RequireMetricsEqual(t, expected, actual)
}
