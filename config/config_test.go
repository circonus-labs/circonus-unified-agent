...
func TestConfig_LoadSingleInputWithEnvVars(t *testing.T) {
	...
	assert.NoError(t, err)
	_ = c.LoadConfig("./testdata/single_plugin_env_vars.toml")
...
func TestConfig_LoadSingleInput(t *testing.T) {
	c := NewConfig()
	_ = c.LoadConfig("./testdata/single_plugin.toml")
...
func TestConfig_BadOrdering(t *testing.T) {
	...
	assert.Equal(t, "Error loading config file ./testdata/non_slice_slice.toml: error parsing file array, line 4: cannot unmarshal TOML array into string (need slice)", err.Error())
}
...
// 	assert.Equal(t, "Circonus/", azureMonitor.NamespacePrefix)
// 	assert.Equal(t, true, ok)
