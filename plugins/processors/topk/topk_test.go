...
// Key, value pair that represents a cua.Metric Tag
type tag struct {
	key string
	val string
}
...
type metricChange struct {
	...
	// This happens because cua.Metric maintains an internal cache of
...
func deepCopy(a []cua.Metric) []cua.Metric {
	ret := make([]cua.Metric, 0, len(a))
	for _, m := range a {
...
func belongs(m cua.Metric, ms []cua.Metric) bool {
	for _, i := range ms {
...
func subSet(a []cua.Metric, b []cua.Metric) bool {
	subset := true
	for _, m := range a {
		if !belongs(m, b) {
			subset = false
...
func equalSets(l1 []cua.Metric, l2 []cua.Metric) bool {
...
func runAndCompare(topk *TopK, metrics []cua.Metric, answer []cua.Metric, testID string, t *testing.T) {
