...
func TestAllScriptTestData(t *testing.T) {
	...
	for _, testdataPath := range paths {
		_ = filepath.Walk(testdataPath, func(path string, info os.FileInfo, err error) error {
			...
			t.Run(fn, func(t *testing.T) {
				...
				testutil.RequireMetricsEqual(t, outputMetrics, acc.GetCUAMetrics(), testutil.SortMetrics(), testutil.IgnoreTime())
			})
			return nil
		})
	}
...
func parseMetricsFrom(t *testing.T, lines []string, header string) (metrics []cua.Metric) {
