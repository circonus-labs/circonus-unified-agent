...
func (r *ReverseDNS) Start(acc cua.Accumulator) error {
	r.acc = acc
...
func (r *ReverseDNS) Add(metric cua.Metric, acc cua.Accumulator) error {
	r.parallel.Enqueue(metric)
	return nil
}

func (r *ReverseDNS) asyncAdd(metric cua.Metric) []cua.Metric {
	...
	return []cua.Metric{metric}
}

func init() {
	processors.AddStreaming("reverse_dns", func() cua.StreamingProcessor {
