...
// addJSONCounter adds a counter array to an Accumulator, with the specified tags.
func addJSONCounter(acc cua.Accumulator, commonTags map[string]string, stats map[string]int) {
...
// addStatsJson walks a jsonStats struct and adds the values to the cua.Accumulator.
func (b *Bind) addStatsJSON(stats jsonStats, acc cua.Accumulator, urlTag string) {
	grouper := metric.NewSeriesGrouper()
	ts := time.Now()
...
func (b *Bind) readStatsJSON(addr *url.URL, acc cua.Accumulator) error {
	var stats jsonStats
