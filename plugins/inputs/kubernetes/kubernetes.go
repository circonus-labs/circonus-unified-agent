...
func init() {
	inputs.Add("kubernetes", func() cua.Input {
		return &Kubernetes{
			LabelInclude: []string{},
			LabelExclude: []string{"*"},
		}
...
func buildSystemContainerMetrics(summaryMetrics *SummaryMetrics, acc cua.Accumulator) {
...
func buildNodeMetrics(summaryMetrics *SummaryMetrics, acc cua.Accumulator) {
	tags := map[string]string{
		"node_name": summaryMetrics.Node.NodeName,
	}
	fields := make(map[string]interface{})
