...
func (t *TopK) Reset() {
	t.cache = make(map[string][]cua.Metric)
...
func (t *TopK) generateGroupByKey(m cua.Metric) (string, error) {
...
func (t *TopK) groupBy(m cua.Metric) {
	...
	if _, ok := t.cache[groupkey]; !ok {
		t.cache[groupkey] = make([]cua.Metric, 0, 10)
	}
...
func (t *TopK) Apply(in ...cua.Metric) []cua.Metric {
...
func (t *TopK) push() []cua.Metric {
	...
	result := make([]cua.Metric, 0, len(ret))
	for _, m := range ret {
...
func (t *TopK) getAggregationFunction(aggOperation string) (func([]cua.Metric, []string) map[string]float64, error) {
	// This is a function aggregates a set of metrics using a given aggregation function
	var aggregator = func(ms []cua.Metric, fields []string, f func(map[string]float64, float64, string)) map[string]float64 {
...
func init() {
	processors.Add("topk", func() cua.Processor {
		return New()
	})
}
