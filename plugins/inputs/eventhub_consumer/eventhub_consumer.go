...
func (*EventHub) SampleConfig() string {
  ...
  ## Change to set a custom user agent, "circonus-unified-agent" is used by default
  # user_agent = "circonus-unified-agent"
...
func (e *EventHub) onDelivery(
	...
	groups map[cua.TrackingID][]cua.Metric,
	track cua.DeliveryInfo,
) bool {
	if track.Delivered() {
...
func (e *EventHub) startTracking(ctx context.Context, ac cua.Accumulator) {
	acc := ac.WithTracking(e.MaxUndeliveredMessages)
	sem := make(semaphore, e.MaxUndeliveredMessages)
	groups := make(map[cua.TrackingID][]cua.Metric, e.MaxUndeliveredMessages)
...
func deepCopyMetrics(in []cua.Metric) []cua.Metric {
	metrics := make([]cua.Metric, 0, len(in))
	for _, m := range in {
...
func init() {
	inputs.Add("eventhub_consumer", func() cua.Input {
		return &EventHub{}
	})
}
