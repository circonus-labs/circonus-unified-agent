...
import (
	...
	tlsConfig "github.com/circonus-labs/circonus-unified-agent/plugins/common/tls"
...
func (s *Syslog) listenPacket(acc cua.Accumulator) {
	defer s.wg.Done()
...
func (s *Syslog) listenStream(acc cua.Accumulator) {
	defer s.wg.Done()
...
func (s *Syslog) handle(conn net.Conn, acc cua.Accumulator) {
	defer func() {
	...
	emit := func(r *syslog.Result) {
		...
		if s.ReadTimeout != nil && s.ReadTimeout.Duration > 0 {
			_ = conn.SetReadDeadline(time.Now().Add(s.ReadTimeout.Duration))
		}
	}
...
func (s *Syslog) store(res syslog.Result, acc cua.Accumulator) {
	if res.Error != nil {
...
func init() {
	inputs.Add("syslog", func() cua.Input {
		return &Syslog{
			Address: ":6514",
			now:     getNanoNow,
			ReadTimeout: &internal.Duration{
