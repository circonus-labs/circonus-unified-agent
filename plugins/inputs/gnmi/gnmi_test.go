	...
	}{
		{
			...
			server: &MockServer{
				SubscribeF: func(server gnmi.GNMI_SubscribeServer) error {
					...
					_ = server.Send(&gnmi.SubscribeResponse{Response: &gnmi.SubscribeResponse_Update{Update: notification}})
					_ = server.Send(&gnmi.SubscribeResponse{Response: &gnmi.SubscribeResponse_SyncResponse{SyncResponse: true}})
...
func TestSubscribeResponseError(t *testing.T) {
	...
	errorResponse := &gnmi.SubscribeResponse_Error{
		Error: &gnmi.Error{Message: me, Code: mc}, //nolint:staticcheck
	}
