...
func TestSerializeMetricBool(t *testing.T) {
	...
	expS := `{"_value":1,"container-name":"cua-test","metric_name":"docker.oomkiller","time":0}`
	assert.Equal(t, string(expS), string(buf))
}
...
func TestSerializeMetricBoolHec(t *testing.T) {
	...
	expS := `{"time":0,"fields":{"_value":0,"container-name":"cua-test","metric_name":"docker.oomkiller"}}`
	assert.Equal(t, string(expS), string(buf))
}
