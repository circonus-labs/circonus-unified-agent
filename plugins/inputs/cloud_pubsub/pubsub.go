...
func (ps *PubSub) onMessage(ctx context.Context, msg message) error {
	...
	if ps.undelivered == nil {
		ps.undelivered = make(map[cua.TrackingID]message)
	}
...
func (ps *PubSub) removeDelivered(id cua.TrackingID) message {
	ps.Lock()
	defer ps.Unlock()
...
func init() {
	inputs.Add("cloud_pubsub", func() cua.Input {
		ps := &PubSub{
...
const sampleConfig = `
  ...
  ## PubSub APIs. If not set explicitly, the agent will attempt to use
  ...
  ## Optional. If true, the agent will attempt to base64 decode the 
