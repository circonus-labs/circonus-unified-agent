...
func initQueries(s *SQLServer) error {
	...
	case "SQLServer": // These are still V2 queries and have not been refactored yet.
		...
		queries["SqlStoredProcedures"] = Query{ScriptName: "SqlStoredProcedures", Script: sqlStoredProceduresV2, ResultByRow: false}
...
func (s *SQLServer) gatherServer(server string, query Query, acc cua.Accumulator) error {
	// deferred opening
...
func (s *SQLServer) accRow(query Query, acc cua.Accumulator, row scanner) error {
...
func init() {
	inputs.Add("sqlserver", func() cua.Input {
		return &SQLServer{}
	})
}
