...
import (
	...
	internalaws "github.com/circonus-labs/circonus-unified-agent/config/aws"
...
type (
	...
	KinesisConsumer struct {
		...
		records       map[cua.TrackingID]string
...
var sampleConfig = `
  ...
  ## Kinesis StreamName must exist prior to starting circonus-unified-agent.
...
func (k *KinesisConsumer) connect(ac cua.Accumulator) error {
	...
	k.records = make(map[cua.TrackingID]string, k.MaxUndeliveredMessages)
...
func init() {
	...
	inputs.Add("kinesis_consumer", func() cua.Input {
