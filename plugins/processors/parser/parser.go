...
func (p *Parser) Apply(metrics ...cua.Metric) []cua.Metric {
	if p.Parser == nil {
		var err error
...
func merge(base cua.Metric, metrics []cua.Metric) cua.Metric {
	for _, metric := range metrics {
		for _, field := range metric.FieldList() {
...
func (p *Parser) parseField(value string) ([]cua.Metric, error) {
	return p.Parser.Parse([]byte(value))
}

func init() {
	processors.Add("parser", func() cua.Processor {
