...
// Suricata is an input plugin for Suricata runtime statistics.
...
func (s *Suricata) handleServerConnection(ctx context.Context, acc cua.Accumulator) {
	var err error
	for {
		select {
		case <-ctx.Done():
...
func init() {
	inputs.Add("suricata", func() cua.Input {
		return &Suricata{
			Source:    "/var/run/suricata-stats.sock",
			Delimiter: "_",
		}
