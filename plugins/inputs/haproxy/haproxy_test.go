	...
	for {
		...
		go func(c net.Conn) {
			...
			if string(data) == "show stat\n" {
				_, _ = c.Write([]byte(csvOutputSample))
				c.Close()
			}
		}(conn)
	}
...
func TestHaproxyGeneratesMetricsUsingSocket(t *testing.T) {
	...
	_badmask := filepath.Join(os.TempDir(), "/tmp/test-fail-haproxy*.sock")

	for i := 0; i < 5; i++ {
		_ = binary.Read(rand.Reader, binary.LittleEndian, &randomNumber)
		sockname := fmt.Sprintf(filepath.Join(os.TempDir(), "test-haproxy%d.sock"), randomNumber)
