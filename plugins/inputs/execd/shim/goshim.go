...
var (
	oldpkg = "github.com/circonus-labs/circonus-unified-agent/plugins/inputs/execd/shim"
	newpkg = "github.com/circonus-labs/circonus-unified-agent/plugins/common/shim"
)
...
func (s *Shim) AddInputs(newInputs []cua.Input) error {
...
func (s *Shim) Run(pollInterval time.Duration) error {
	...
	s.metricCh = make(chan cua.Metric, 1)
	...
	for _, input := range s.Inputs {
		...
		if serviceInput, ok := input.(cua.ServiceInput); ok {
...
func (s *Shim) startGathering(ctx context.Context, input cua.Input, acc cua.Accumulator, gatherPromptCh <-chan empty, pollInterval time.Duration) {
...
func DefaultImportedPlugins() (i []cua.Input, e error) {
...
func LoadConfig(filePath *string) ([]cua.Input, error) {
...
func loadConfigIntoInputs(md toml.MetaData, inputConfigs map[string][]toml.Primitive) ([]cua.Input, error) {
	renderedInputs := []cua.Input{}
