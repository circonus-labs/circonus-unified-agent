...
When gathering from the local system, the agent will need permission to the
ipmi device node.  When using udev you can create the device node giving
`rw` permissions to the `cua` user by adding the following rule to
`/etc/udev/rules.d/52-circonus-unified-agent-ipmi.rules`:
...
cua  ALL=(root) NOPASSWD: IPMITOOL
