...
// about the agent. Metrics can be registered using this package, and then
...
// Stat is an interface for dealing with agent statistics collected
// on itself.
type Stat interface {
...
// value will be returned as an agent metric when Metrics() is called.
...
// Metrics returns all registered stats as agent metrics.
func Metrics() []cua.Metric {
	registry.mu.Lock()
	now := time.Now()
	metrics := make([]cua.Metric, len(registry.stats))
	i := 0
