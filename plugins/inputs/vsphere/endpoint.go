...
var (
	isolateLUN = regexp.MustCompile(".*/([^/]+)/?$")
	isIPv4     = regexp.MustCompile(`^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$`)
	isIPv6     = regexp.MustCompile("^(?:[A-Fa-f0-9]{0,4}:){1,7}[A-Fa-f0-9]{1,4}$")
)

const (
	//  rtMetricLookback = 3 // Number of time periods to look back at for realtime metrics
	metricLookback     = 3    // Number of time periods to look back at for non-realtime metrics
...
// as parameters.
func NewEndpoint(ctx context.Context, parent *VSphere, url *url.URL, log cua.Logger) (*Endpoint, error) {
	e := Endpoint{
		URL:               url,
		Parent:            parent,
...
func (e *Endpoint) Collect(ctx context.Context, acc cua.Accumulator) error {
...
func (e *Endpoint) chunkify(ctx context.Context, res *resourceKind, now time.Time, latest time.Time, job queryJob) {
	...
	for _, object := range res.objects {
		...
		for _, bucket := range timeBuckets {
			...
			if (!res.realTime && len(pqs) > e.Parent.MaxQueryObjects) || len(pqs) > maxRealtimeMetrics {
...
func (e *Endpoint) collectResource(ctx context.Context, resourceType string, acc cua.Accumulator) error {
