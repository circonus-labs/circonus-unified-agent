...
will scrape pods only in the node that circonus-unified-agent is running. It will fetch the
pod list locally from the node's kubelet. This will require running circonus-unified-agent in
every node of the cluster. Note that either `node_ip` must be specified in the
config or the environment variable `NODE_IP` must be set to the host IP. The
latter can be done in the yaml of the pod running circonus-unified-agent:
...
The pod running circonus-unified-agent will need to have the proper rbac configuration in
...
agent configuration. If using Kubernetes service discovery the `address`
