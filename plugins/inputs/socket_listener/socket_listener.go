	...
	for {
		...
		if ssl.ReadBufferSize.Size > 0 {
			if srb, ok := c.(setReadBufferer); ok {
				_ = srb.SetReadBuffer(int(ssl.ReadBufferSize.Size))
			} else {
...
func (ssl *streamSocketListener) read(c net.Conn) {
	...
	for {
		if ssl.ReadTimeout != nil && ssl.ReadTimeout.Duration > 0 {
			_ = c.SetReadDeadline(time.Now().Add(ssl.ReadTimeout.Duration))
		}
		if !scnr.Scan() {
			break
		}
	...
	case "udp", "udp4", "udp6", "ip", "ip4", "ip6", "unixgram":
		...
		if sl.ReadBufferSize.Size > 0 {
			if srb, ok := pc.(setReadBufferer); ok {
				_ = srb.SetReadBuffer(int(sl.ReadBufferSize.Size))
			} else {
...
func init() {
	inputs.Add("socket_listener", func() cua.Input { return newSocketListener() })
}
