...
func remove(slice []cua.Metric, i int) []cua.Metric {
...
// Save item to cache
func (d *Dedup) save(metric cua.Metric, id uint64) {
	d.Cache[id] = metric.Copy()
	d.Cache[id].Accept()
}

// main processing method
func (d *Dedup) Apply(metrics ...cua.Metric) []cua.Metric {
...
func init() {
	processors.Add("dedup", func() cua.Processor {
		return &Dedup{
