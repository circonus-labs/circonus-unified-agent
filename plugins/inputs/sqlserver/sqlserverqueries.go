...
IF SERVERPROPERTY('EngineEdition') NOT IN (2,3,4) BEGIN /*NOT IN Standard,Enterpris,Express*/
	DECLARE @ErrorMessage AS nvarchar(500) = 'Circonus Unified Agent - Connection string Server:'+ @@ServerName + ',Database:' + DB_NAME() +' is not a SQL Server Standard,Enterprise or Express. Check the database_type parameter in the configuration.';
...
IF @MajorMinorVersion >= 1050 BEGIN
	/*in [volume_mount_point] any trailing "\" char will be automatically removed by agent */
	SET @Columns += N'
	,[volume_mount_point]'
	SET @Tables += N'
...
IF @MajorMinorVersion >= 1050 BEGIN
	SELECT DISTINCT
		...
		,REPLACE(@@SERVERNAME,'\',':') AS [sql_instance]
		/*in [volume_mount_point] any trailing "\" char will be removed by agent */
