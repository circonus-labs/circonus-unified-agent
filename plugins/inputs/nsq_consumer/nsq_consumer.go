...
type NSQConsumer struct {
	...
	mu       sync.Mutex
	messages map[cua.TrackingID]*nsq.Message
	wg       sync.WaitGroup
...
func (n *NSQConsumer) Start(ac cua.Accumulator) error {
	...
	if len(n.Nsqlookupd) > 0 {
		_ = n.consumer.ConnectToNSQLookupds(n.Nsqlookupd)
	}
	_ = n.consumer.ConnectToNSQDs(append(n.Nsqd, n.Server))
...
func (n *NSQConsumer) onDelivery(ctx context.Context, acc cua.TrackingAccumulator, sem semaphore) {
	for {
		select {
		case <-ctx.Done():
			return
...
func init() {
	inputs.Add("nsq_consumer", func() cua.Input {
		return &NSQConsumer{
