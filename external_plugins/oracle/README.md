...
This repository contains a plugin to collect metrics from Oracle RDBMS using Dynamic Performance Views.

## Installation

The plugin executes a python script to gather metrics. It requires proper installation of [python3](https://www.python.org/downloads/) with [cx_Oracle](https://cx-oracle.readthedocs.io/en/latest/user_guide/installation.html) extension module, which in turn also requires installation of [Oracle Client](https://www.oracle.com/database/technologies/instant-client/downloads.html). The Oracle client might require extra environment variables, which must be set before `circonus-unified-agent` is executed.

### Modify circonus-unified-agent configuration

e.g. `/opt/circonus/unified-agent/etc/circonus-unified-agent.conf`

```toml
[[inputs.exec]]
  # set to a unique name
  instance_id = "oracle" 
  # path to the install of circonus-unified-agent external plugins directory
  # e.g. commands["/opt/circonus/unified-agent/external_plugins/oracle/oracle_metrics.sh]
  commands = ["/path/to/external_plugins/oracle/oracle_metrics.sh"] # 
  timeout = "60s"
  data_format = "influx"
...
The `oracle_metrics.sh` script executes the python script and also ensures that Oracle
user password is not printed in logs.

### Create `/path/to/external_plugins/oracle/config.sh`

This is created as a separate file (rather than modifying oracle_metrics.sh directly), so that it is not overwritten by an update of the circonus-unified-agent.

* Set [connection string](https://cx-oracle.readthedocs.io/en/latest/user_guide/connection_handling.html#connection-strings) to your database.
* Set username and password of your Oracle user with SELECT_CATALOG_ROLE role granted.
* Set the value of `instance` tag of the generated metrics.

e.g. `/opt/circonus/unified-agent/external_plugins/oracle/config.sh`
...
```shell
$ cd /path/to/circonus-unified-agent 
$ export LD_LIBRARY_PATH=/opt/oracle/client
$ sbin/circonus-unified-agent --config etc/circonus-unified-agent.conf --input-filter exec --test
...
[https://github.com/bonitoo-io/telegraf-input-oracle](https://github.com/bonitoo-io/telegraf-input-oracle) for updated version of this plugin.
[https://github.com/wavefrontHQ/integrations/oracle](https://github.com/wavefrontHQ/integrations/tree/33c92c3/oracle) was initially used to create this plugin.
