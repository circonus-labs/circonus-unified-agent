...
func assertEqual(t *testing.T, exp, actual []cua.Metric) {
...
func TestTryAddState(t *testing.T) {
	tests := []struct {
		...
		runErrF func() error
		metrics []cua.Metric
		assertF func(*testing.T, []cua.Metric, error)
	}{
		{
			...
			assertF: func(t *testing.T, metrics []cua.Metric, err error) {
				exp := []cua.Metric{
					mb().
						n("nagios").
						f("perfdata", 0).b(),
...
func assertNagiosState(t *testing.T, m cua.Metric, f map[string]interface{}) {
