...
func (p *Parser) Parse(input []byte) ([]cua.Metric, error) {
...
func (p *Parser) processMetric(input []byte, data []DataSet, tag bool, timestamp time.Time) ([]cua.Metric, error) {
	if len(data) == 0 {
		return nil, nil
	}
...
func cartesianProduct(a, b []cua.Metric) []cua.Metric {
	if len(a) == 0 {
		return b
	}
	if len(b) == 0 {
...
func mergeMetric(a cua.Metric, m cua.Metric) {
...
func (p *Parser) expandArray(result MetricNode, timestamp time.Time) ([]cua.Metric, error) {
	var results []cua.Metric
...
func (p *Parser) processObjects(input []byte, objects []JSONObject, timestamp time.Time) ([]cua.Metric, error) {
...
func (p *Parser) combineObject(result MetricNode, timestamp time.Time) ([]cua.Metric, error) {
	var results []cua.Metric
	if result.IsArray() || result.IsObject() {
		var err error
