...
func (*Elasticsearch) Description() string {
	return "Configuration for ElasticSearch output plugin."
}
...
func GetPointID(m cua.Metric) string {
...
func (a *Elasticsearch) Write(metrics []cua.Metric) (int, error) {
	numMetrics := -1
	...
	for _, metric := range metrics {
		...
		// index name has to be re-evaluated each time for circonus-unified-agent
	...
	if err != nil {
		return numMetrics, fmt.Errorf("error sending bulk request to Elasticsearch: %w", err)
	}
	...
	if res.Errors {
		...
		return numMetrics, fmt.Errorf("elasticsearch failed to index %d metrics", len(res.Failed()))
	}
...
func (a *Elasticsearch) manageTemplate(ctx context.Context) error {
	...
	templateExists, errExists := a.Client.IndexTemplateExists(a.TemplateName).Do(ctx) //nolint
	...
	if (a.OverwriteTemplate) || (!templateExists) || (templatePattern != "") {
		...
		t := template.Must(template.New("template").Parse(circonusTemplate))
		var tmpl bytes.Buffer

		if err := t.Execute(&tmpl, tp); err != nil {
			return err
		}
		_, errCreateTemplate := a.Client.IndexPutTemplate(a.TemplateName).BodyString(tmpl.String()).Do(ctx) //nolint
...
func init() {
	outputs.Add("elasticsearch", func() cua.Output {
