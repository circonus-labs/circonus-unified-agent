...
func collectIngress(ctx context.Context, acc cua.Accumulator, ki *KubernetesInventory) {
...
func (ki *KubernetesInventory) gatherIngress(i netv1.Ingress, acc cua.Accumulator) {
	if i.GetCreationTimestamp().Second() == 0 && i.GetCreationTimestamp().UnixNano() == 0 {
		return
	}
	...
	for _, ingress := range i.Status.LoadBalancer.Ingress {
		...
		for _, rule := range i.Spec.Rules {
			for _, path := range rule.IngressRuleValue.HTTP.Paths {
				fields["backend_service_port"] = path.Backend.Service.Port
