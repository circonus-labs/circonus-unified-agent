...
func (s *Solr) Gather(acc cua.Accumulator) error {
	...
	for _, serv := range s.Servers {
		go func(serv string, acc cua.Accumulator) {
			defer wg.Done()
...
func (s *Solr) gatherServerMetrics(server string, acc cua.Accumulator) error {
	...
	for _, core := range cores {
		go func(server string, core string, acc cua.Accumulator) {
			defer wg.Done()
...
func addAdminCoresStatusToAcc(acc cua.Accumulator, adminCoreStatus *AdminCoresStatus, time time.Time) {
...
func addCoreMetricsToAcc(acc cua.Accumulator, core string, mBeansData *MBeansData, time time.Time) error {
...
func addQueryHandlerMetricsToAcc(acc cua.Accumulator, core string, mBeansData *MBeansData, time time.Time) error {
...
func addUpdateHandlerMetricsToAcc(acc cua.Accumulator, core string, mBeansData *MBeansData, time time.Time) error {
...
func addCacheMetricsToAcc(acc cua.Accumulator, core string, mBeansData *MBeansData, time time.Time) error {
...
func init() {
	inputs.Add("solr", func() cua.Input {
		return NewSolr()
	})
}
