...
func (r *Riak) gatherServer(s string, acc cua.Accumulator) error {
	// Parse the given URL to extract the server tag
	u, err := url.Parse(s)
	if err != nil {
		return fmt.Errorf("riak unable to parse given server url %s: %w", s, err)
	}

	// Perform the GET request to the riak /stats endpoint
	resp, err := r.client.Get(s + "/stats")
	if err != nil {
		return fmt.Errorf("client get (%s/stats): %w", s, err)
	}
...
func init() {
	inputs.Add("riak", func() cua.Input {
		return NewRiak()
	})
}
