...
import (
	...
	"github.com/circonus-labs/circonus-unified-agent/plugins/parsers/wavefront"
)
...
type Parser interface {
	...
	// and parses it into metrics
	//
	// Must be thread-safe.
	Parse(buf []byte) ([]cua.Metric, error)

	// ParseLine takes a single string metric
	// ie, "cpu.usage.idle 90"
	// and parses it into a metric.
	//
	// Must be thread-safe.
	ParseLine(line string) (cua.Metric, error)
