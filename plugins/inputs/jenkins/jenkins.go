...
// SampleConfig implements Input interface
func (j *Jenkins) SampleConfig() string {
	return sampleConfig
}

// Description implements Input interface
...
func (j *Jenkins) gatherNodeData(n node, acc cua.Accumulator) error {
...
func (j *Jenkins) gatherNodesData(acc cua.Accumulator) {
...
func (j *Jenkins) gatherJobs(acc cua.Accumulator) {
	...
	for _, job := range js.Jobs {
		wg.Add(1)
		go func(name string, wg *sync.WaitGroup, acc cua.Accumulator) {
			defer wg.Done()
...
func (j *Jenkins) getJobDetail(jr jobRequest, acc cua.Accumulator) error {
	...
	for k, ij := range js.Jobs {
		...
		go func(ij innerJob, jr jobRequest, acc cua.Accumulator) {
			defer wg.Done()
			if err := j.getJobDetail(jobRequest{
				name:    ij.Name,
				parents: jr.combined(),
...
func (j *Jenkins) gatherJobBuild(jr jobRequest, b *buildResponse, acc cua.Accumulator) {
