...
// Wrap updates the starlark.Metric to wrap a new cua.Metric.
func (m *Metric) Wrap(metric cua.Metric) {
	m.metric = metric
	m.tagIterCount = 0
	m.fieldIterCount = 0
	m.frozen = false
...
// Unwrap removes the cua.Metric from the startlark.Metric.
func (m *Metric) Unwrap() cua.Metric {
	return m.metric
}
