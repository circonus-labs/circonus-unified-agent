...
func (a *Agent) Run(ctx context.Context) error {
	...
	iu, err := a.startInputs(ctx, next, a.Config.Inputs)
	if err != nil {
		return err
	}
...
func (a *Agent) startInputs(
	...
	for _, input := range inputs {
		if si, ok := input.Input.(cua.ServiceInput); ok {
...
func (a *Agent) startProcessors(
...
) (chan<- cua.Metric, []*processorUnit, error) {
	...
	var src chan cua.Metric
	for _, processor := range processors {
		src = make(chan cua.Metric, 100)
...
func (a *Agent) startAggregators(
	...
	outputC chan<- cua.Metric,
	aggregators []*models.RunningAggregator,
) (chan<- cua.Metric, *aggregatorUnit) {
	src := make(chan cua.Metric, 100)
	unit := &aggregatorUnit{
		src:         src,
		aggC:        aggC,
...
func (a *Agent) startOutputs(
	ctx context.Context,
	outputs []*models.RunningOutput,
) (chan<- cua.Metric, *outputUnit, error) {
	src := make(chan cua.Metric, 100)
...
// inputs to run.
func (a *Agent) test(ctx context.Context, wait time.Duration, outputC chan<- cua.Metric) error {
	log.Printf("D! [agent] Initializing plugins")
	err := a.initPlugins()
	if err != nil {
		return err
	...
	iu := a.testStartInputs(ctx, next, a.Config.Inputs)
...
func panicRecover(input *models.RunningInput) {
	if err := recover(); err != nil {
		...
		log.Println("E! PLEASE REPORT THIS PANIC ON GITHUB with " +
			...
			"https://github.com/circonus-labs/circonus-unified-agent/issues/new/choose")
	}
}
