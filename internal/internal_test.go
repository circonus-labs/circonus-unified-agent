...
func TestSize(t *testing.T) {
	...
	_ = s.UnmarshalTOML([]byte(`"1B"`))
	assert.Equal(t, int64(1), s.Size)

	s = Size{}
	_ = s.UnmarshalTOML([]byte(`1`))
	assert.Equal(t, int64(1), s.Size)
	...
	s = Size{}
	_ = s.UnmarshalTOML([]byte(`"1GB"`))
	assert.Equal(t, int64(1000*1000*1000), s.Size)

	s = Size{}
	_ = s.UnmarshalTOML([]byte(`"12GiB"`))
...
func TestVersionAlreadySet(t *testing.T) {
	...
	assert.Error(t, err)
	assert.IsType(t, ErrVersionAlreadySet, err)
...
func TestProductToken(t *testing.T) {
	token := ProductToken()
	// Agent version depends on the call to SetVersion, it cannot be set
	// multiple times and is not thread-safe.
	re := regexp.MustCompile(`^circonus-unified-agent/[^\s]+ Go/\d+.\d+(.\d+)?$`)
