...
func newTrackingID() cua.TrackingID {
	return cua.TrackingID(atomic.AddUint64(&lastID, 1))
}
...
func newTrackingMetric(metric cua.Metric, fn NotifyFunc) (cua.Metric, cua.TrackingID) {
	m := &trackingMetric{
		Metric: metric,
		d: &trackingData{
			id:          newTrackingID(),
...
func newTrackingMetricGroup(group []cua.Metric, fn NotifyFunc) ([]cua.Metric, cua.TrackingID) {
	d := &trackingData{
		id:          newTrackingID(),
		rc:          0,
		acceptCount: 0,
...
func (m *trackingMetric) Copy() cua.Metric {
	m.d.incr()
...
func (r *deliveryInfo) ID() cua.TrackingID {
	return r.id
}
