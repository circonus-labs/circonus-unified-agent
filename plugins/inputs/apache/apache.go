...
func (n *Apache) gatherURL(addr *url.URL, acc cua.Accumulator) error {
	req, err := http.NewRequest("GET", addr.String(), nil)
	if err != nil {
		return fmt.Errorf("error on new request to %s : %w", addr.String(), err)
	}
	...
	if err != nil {
		return fmt.Errorf("error on request to %s : %w", addr.String(), err)
	}
...
func init() {
	inputs.Add("apache", func() cua.Input {
		return &Apache{}
	})
}
