...
func TestGatherAttributes(t *testing.T) {
	...
	t.Run("Smartctl presence", func(t *testing.T) {
		...
		t.Run("Only nvme device", func(t *testing.T) {
			...
			testutil.RequireMetricsEqual(t, testSmartctlNvmeAttributes, acc.GetCUAMetrics(),
...
func TestGatherHtSAS(t *testing.T) {
	...
	testutil.RequireMetricsEqual(t, testHtsasAtributtes, acc.GetCUAMetrics(), testutil.SortMetrics(), testutil.IgnoreTime())
}
...
var (
	testOverflowAttributes = []cua.Metric{
		testutil.MustMetric(
			"smart_attribute",
			map[string]string{
				"device": "nvme0",
	...
	testHtsasAtributtes = []cua.Metric{
		testutil.MustMetric(
			"smart_attribute",
			map[string]string{
				"device":    ".",
	...
	testSmartctlNvmeAttributes = []cua.Metric{
	...
	testIntelInvmeAttributes = []cua.Metric{
		testutil.MustMetric("smart_attribute",
			map[string]string{
				"device":    "nvme0",
