...
var (
	...
	intelAttributes = map[string]struct {
		ID    string
		Name  string
		Parse func(acc cua.Accumulator, fields map[string]interface{}, tags map[string]string, str string) error
	}{
		"program_fail_count": {
...
var sampleConfig = `
  ...
  ## Sudo must be configured to allow the user to run smartctl or nvme-cli
  ## without a password.
  # use_sudo = false
...
func (m *Smart) getAttributes(acc cua.Accumulator, devices []string) {
...
func (m *Smart) getVendorNVMeAttributes(acc cua.Accumulator, devices []string) {
...
func gatherIntelNVMeDisk(acc cua.Accumulator, timeout internal.Duration, usesudo bool, nvme string, device nvmeDevice, wg *sync.WaitGroup) {
	defer wg.Done()
...
func gatherDisk(acc cua.Accumulator, timeout internal.Duration, usesudo, collectAttributes bool, smartctl, nocheck, device string, wg *sync.WaitGroup) {
	defer wg.Done()
...
func parseBytesWritten(acc cua.Accumulator, fields map[string]interface{}, tags map[string]string, str string) error {
	var value int64
...
func parseThermalThrottle(acc cua.Accumulator, fields map[string]interface{}, tags map[string]string, str string) error {
...
func parseWearLeveling(acc cua.Accumulator, fields map[string]interface{}, tags map[string]string, str string) error {
...
func parseTimedWorkload(acc cua.Accumulator, fields map[string]interface{}, tags map[string]string, str string) error {
	var value float64
...
func init() {
	...
	inputs.Add("smart", func() cua.Input {
		m := newSmart()
		m.Nocheck = "standby"
		return m
	})
